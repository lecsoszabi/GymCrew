...
describe("memberStats", () => {
  ...
  it("csak a saját edzéseket számolja", () => {
    const rows = [checkIn("anna", 1), checkIn("anna", 3), checkIn("bence", 2)];
    expect(memberStats(rows, "anna").total).toBe(2);
    expect(memberStats(rows, "bence").total).toBe(1);
  });

  it("a 7 és 30 napos ablakot helyesen vágja", () => {
    const rows = [checkIn("anna", 1), checkIn("anna", 10), checkIn("anna", 60)];
    const s = memberStats(rows, "anna");
  ...
  it("a legutóbbi látogatás a legfrissebb", () => {
    const rows = [checkIn("anna", 9), checkIn("anna", 2), checkIn("anna", 40)];
    const s = memberStats(rows, "anna");
    expect(new Date(s.lastVisit!).getTime()).toBeGreaterThan(MOST.getTime() - 3 * DAY);
  });

  it("a kedvenc időpont a leggyakoribb óra", () => {
    const rows = [checkIn("anna", 1, 19), checkIn("anna", 3, 19), checkIn("anna", 5, 7)];
    expect(memberStats(rows, "anna").favoriteHour).toBe(19);
  });

  it("a napok eloszlása összeadva kiadja az összeset", () => {
    const rows = [checkIn("anna", 1), checkIn("anna", 4), checkIn("anna", 9)];
    const s = memberStats(rows, "anna");
  ...
  it("három egymást követő hétből hármas sorozat lesz", () => {
    ...
    const rows = [checkIn("anna", 0), checkIn("anna", 7), checkIn("anna", 14)];
    expect(memberStats(rows, "anna").streakWeeks).toBe(3);
  });

  it("a sorozat megszakad, ha kimarad egy hét", () => {
    // Ezen a héten és három hete — a köztes két hét üres.
    const rows = [checkIn("anna", 0), checkIn("anna", 21)];
    expect(memberStats(rows, "anna").streakWeeks).toBe(1);
  });

  it("a futó hét üressége még nem töri meg a sorozatot", () => {
    // Ezen a héten semmi, de az előző kettőben igen — a sorozat él.
    const rows = [checkIn("anna", 7), checkIn("anna", 14)];
    expect(memberStats(rows, "anna").streakWeeks).toBe(2);
  });

  it("csak régi edzésnél nincs sorozat", () => {
    const rows = [checkIn("anna", 60)];
    expect(memberStats(rows, "anna").streakWeeks).toBe(0);
  });
});

describe("weeklySeries", () => {
  it("a kért számú hetet adja vissza", () => {
    expect(weeklySeries([], ["anna"], 8)).toHaveLength(8);
  });

  it("az utolsó oszlop a futó hét", () => {
    expect(weeklySeries([], ["anna"], 4).at(-1)!.label).toBe("Most");
  });

  it("a tagonkénti bontás összege a heti összeg", () => {
    const rows = [checkIn("anna", 1), checkIn("bence", 1), checkIn("anna", 2)];
    for (const b of weeklySeries(rows, ["anna", "bence"], 2)) {
  ...
  it("nem számol bele idegen felhasználót", () => {
    ...
    expect(weeklySeries(rows, ["anna"], 2).reduce((a, b) => a + b.total, 0)).toBe(0);
  });

  it("a megfelelő hetekbe sorolja az edzéseket", () => {
    const rows = [checkIn("anna", 0), checkIn("anna", 7)];
    const bars = weeklySeries(rows, ["anna"], 3);
